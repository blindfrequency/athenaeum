...
Item {
    ...
    function resizePage(width, height) {
        //TODO update idea - resize only when in range of window, when context already bigger don't change it just
    ...
    function loadNewUrl(newUrl, jsonObject){
        ...
        var moment = Date.now() //if we would save it somewhere we can load old cache
        ...
        if (mainLoader.item.requestedWidth)
            ...
            mainFlick.contentWidth = 1280 //hardcode away oneday
    ...
    Timer{
        ...
        onTriggered: {
           ...
           if (hash !== athenumPage.md5hash){
               console.log("Hash changed ",athenumPage.url, " from ", athenumPage.md5hash," to ",hash)
               loadNewUrl(athenumPage.url)
           }
        }
    }
