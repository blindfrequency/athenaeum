...
Item {
    ...
    Button{
     ...
     function calc(){
         sumModel.clear()
         //var elementsCount = parseInt(amountElements.text)
         for (var i = 0; i <= progressionsCount.value; ++i){
             sumModel.addNew(num,prime,10,i,amountElements.value) //elementsCount)
         }
    ...
    TableView{
        ...
        delegate:
            ...
            Text {
            width: 10
            //we can calculate if index % rowCount() == 0 then its first line, then we should bold it and color
            text: display[0] + ' '}
